...
function setLanguage(lang) {
  ...
  // URL (href) များကို ဘာသာစကားအလိုက် ပြောင်းလဲခြင်း
...
function updateActiveButtons(lang) {
  document
    .querySelectorAll(".mobile-lang-bar button, .lang-options button")
    .forEach((btn) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (langBar) {
    langBar.addEventListener("click", (e) => {
      // ၁။ Button ကိုနှိပ်ရင် ဘာသာစကားပြောင်းပြီး ပြန်ပိတ်မယ်
      ...
      // ၂။ Icon ကိုနှိပ်ရင် Menu ကို ဖွင့်/ပိတ် လုပ်မယ်
      if (e.target.closest(".lang-toggle-icon")) {
        langBar.classList.toggle("active");
        e.stopPropagation();
      }
    });

    // ၃။ Menu အပြင်ဘက်ကို နှိပ်ရင် ပြန်ပိတ်မယ်
  ...
  // ၄။ သာမန် Static Language Switcher Buttons များအတွက် Click Event (ဥပမာ Desktop Version အတွက်)
  ...
  langButtons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // Floating bar ထဲက ခလုတ်ဖြစ်ပါက parent trigger မဖြစ်စေရန် stop propagation လုပ်သည်
      if (langBar && langBar.contains(btn)) {
        e.stopPropagation();
      }
    });
  });
